...
#define TASKMANAGER \
  ...
  std::unique_ptr<QSettings> settings(
    new QSettings(TASKMANAGER, QSettings::NativeFormat));
...
loveLetter::loveLetter(QWidget* parent) : QWidget(parent) {
#ifdef Q_OS_WIN
  ...
  pBtn[0]->setGeometry((windowWidth / 2 - btnWidth) / 2,
                       windowHeight - btnHeight - pending, 0, 0);
  pBtn[0]->setText(tr("我愿意！"));
  pBtn[1]->setGeometry(3 * windowWidth / 4 - btnWidth / 2,
                       windowHeight - btnHeight - pending, 0, 0);
  ...
  pAgPA.reset(
    new QPropertyAnimation(pAgEff.get(), "opacity", this->pAgree.get()));
  ...
  pRfPA.reset(
    new QPropertyAnimation(pRfEff.get(), "opacity", this->pRefuse.get()));
...
int loveLetter::whichEdge(int x, int y) {
  if (x < refuseBoxx1 || x > refuseBoxx2 || y < refuseBoxy1 || y > refuseBoxy2)
    goto NONE;
  if (x <= refuseBoxx1 + pending && y <= refuseBoxy1 + pending) return 0;
  if (x > refuseBoxx1 + pending && x < refuseBoxx2 - pending
      && y <= refuseBoxy1 + pending)
    return 1;
  if (x >= refuseBoxx2 - pending && y <= refuseBoxy1 + pending) return 2;
  if (x >= refuseBoxx2 - pending && y > refuseBoxy1 + pending
      && y < refuseBoxy2 - pending)
    return 3;
  ...
  if (y > refuseBoxy1 + pending && x <= refuseBoxx1 + pending
      && y < refuseBoxy2 - pending)
    return 7;
NONE:
  return -1;
